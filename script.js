...
function getHeaderHeight() {
    ...
    // Return actual height if visible, otherwise a default (e.g., for initial calculation)
    return headerElement && headerElement.offsetHeight > 0 ? headerElement.offsetHeight : 70;
}

// --- Function to Open Modals (Imprint/Privacy) ---
function openModal(modalId) {
    ...
    // Ensure other overlays/popups are closed first
    closeCookieBanner(); // Close cookie banner if open
    // Close mobile nav if open (might need toggleNav function)
    const mainNav = document.querySelector('.main-nav');
    if (mainNav && mainNav.classList.contains('active')) {
        const navToggle = document.getElementById('nav-toggle');
        if (navToggle) navToggle.click(); // Simulate click to close nav via its toggle function
    }
    ...
    updateBodyScrollLock(); // Central function to manage scroll lock
}

// --- Function to Close Modals (Imprint/Privacy) ---
function closeModal() {
    const modalOverlay = document.getElementById('modal-overlay');
    ...
    updateBodyScrollLock(); // Update scroll lock state
}
...
document.addEventListener('DOMContentLoaded', function() {
    try { // Wrap potentially error-prone code in try...catch
        ...
        Object.keys(i18nStrings).forEach(key => {
            ...
            // --- SAFEGUARD: Check if element exists ---
            if (!element) {
                // console.warn(`Element with data-i18n-key="${key}" not found.`);
                return; // Skip this key if element is missing
            }
            ...
            if (translation !== undefined) {
                ...
                // Use innerHTML for specific keys, textContent for others
        ...
        initializePage(); // Initialize core functionality AFTER translations
        initializeCookieBanner(); // Initialize cookie banner
    } catch (error) {
        console.error("Error during initialization:", error);
        // Display a fallback message to the user if initialization fails catastrophically
...
function initializePage() {
    // --- Elements (re-select within functions or ensure they exist) ---
    const body = document.body;
    const modalOverlay = document.getElementById('modal-overlay'); // Shared overlay
    const navToggle = document.getElementById('nav-toggle');
    const mainNav = document.querySelector('.main-nav');
    ...
    // --- Initializations ---
    if (typeof feather !== 'undefined') feather.replace(); // Check if feather exists
    if (typeof AOS !== 'undefined') AOS.init({ duration: 800, easing: 'ease-in-out-quad', once: true, offset: 50 }); // Check if AOS exists
    ...
    const toggleNav = () => {
        ...
        if (isOpening) {
            modalOverlay.classList.add('active'); // Show shared overlay
    ...
    const updateNavToggleIconColor = (isOpen) => { /* ... (keep existing code) ... */
    ...
    if (navToggle && mainNav && modalOverlay) {
         ...
         // Listener for links *inside* the mobile nav
         mainNav.querySelectorAll('a[href^="#"]').forEach(link => {
            link.addEventListener('click', (e) => {
                const targetId = link.hash;
                // Check if the target actually exists on the page
                 if (targetId && targetId !== '#' && document.getElementById(targetId.substring(1))) {
                     ...
                     if (mainNav.classList.contains('active')) {
                         ...
                         // If nav somehow not active but link clicked, scroll directly
                         smoothScrollToTarget(targetId);
                     }
                 }
                 // Allow default for external links or links like href="#"
            });
         });
     }
    ...
    // Listener for non-nav anchor clicks (buttons etc.)
    document.addEventListener('click', function(e) {
        const link = e.target.closest('a');
        // Check: is it a link? does it have a hash? is it internal? is the target valid? is it NOT in the mobile nav?
    ...
    const animateCountersObserver = new IntersectionObserver((entries) => { /* ... (keep existing code, should be fine) ... */
        entries.forEach(entry => {
            if (entry.isIntersecting && !countersAnimated && entry.target.id === 'experience') {
                ...
                if (experienceSection) animateCountersObserver.unobserve(experienceSection); // Unobserve after animating
            }
        });
    }, { threshold: 0.3 });
    ...
    // --- Modal Event Listeners (Imprint/Privacy) ---
    if (openImprintLink && imprintModal) {
        openImprintLink.addEventListener('click', (e) => {
            e.preventDefault();
            openModal('imprint-modal'); // Use the function defined earlier
        });
    }
    if (openPrivacyLink && privacyModal) {
        openPrivacyLink.addEventListener('click', (e) => {
            e.preventDefault();
            openModal('privacy-modal'); // Use the function defined earlier
        });
    }
    // Add listener to close buttons within modals
    closeButtons.forEach(button => {
        button.addEventListener('click', closeModal); // Use the function defined earlier
    });

    // Listener for the main overlay (closes modals and nav)
     if (modalOverlay) {
         modalOverlay.addEventListener('click', () => {
             if (isModalOpen()) closeModal();
             else if (isNavOpen()) toggleNav(); // Use toggleNav to handle state correctly
         });
     }
    ...
    document.addEventListener('keydown', (event) => {
        if (event.key === "Escape") {
            if (isModalOpen()) closeModal();
            else if (isNavOpen()) toggleNav(); // Use toggleNav
            else if (isCookiePopupOpen()) closeCookieBanner(); // Use dedicated close function
        }
    });
...
function initializeCookieBanner() {
    // --- SAFEGUARD: Ensure elements exist before adding listeners ---
    const banner = document.getElementById('cookie-banner');
    const overlay = document.getElementById('cookie-overlay');
    const acceptButton = document.getElementById('cookie-accept');
    const privacyLinkInBanner = document.getElementById('cookie-banner-privacy-link');

    if (!banner || !overlay || !acceptButton || !privacyLinkInBanner) {
        console.warn("Cookie banner elements not found. Banner initialization skipped.");
    ...
    const handleConsent = () => {
        try { // Safeguard localStorage access
            localStorage.setItem(CONSENT_KEY, CONSENT_VALUE);
            closeCookieBanner(); // Use the dedicated close function
        } catch (e) {
            console.error("Could not set cookie consent in localStorage:", e);
            // Fallback: just hide the banner visually for this session
             closeCookieBanner();
        }
    }
    ...
    if (currentConsent !== CONSENT_VALUE) {
        ...
        // Link to open privacy modal from banner
         privacyLinkInBanner.addEventListener('click', (e) => {
             ...
             // Need a slight delay to allow close transition
             setTimeout(() => {
                 openModal('privacy-modal'); // Use the global modal open function
             }, 350); // Delay slightly longer than transition
         });

          // Optional: Close banner if overlay is clicked (user might expect this)
         overlay.addEventListener('click', handleConsent);

    }
    // No 'else' needed - if consent exists, banner remains hidden by default CSS
